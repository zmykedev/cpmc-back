...
export const CMPC_SPECIFIC_GENRES = CMPC_GENRE_DESCRIPTIONS
  .filter(g => g.category === 'CMPC_SPECIFIC')
  .map(g => g.genre);

export const TRADITIONAL_GENRES = Object.values(Genre)
  .filter(genre => !CMPC_SPECIFIC_GENRES.includes(genre));
