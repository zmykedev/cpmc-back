...
export const CMPC_SPECIFIC_AUTHORS = CMPC_AUTHOR_DESCRIPTIONS
  .filter(a => a.category === 'CMPC_SPECIFIC')
  .map(a => a.author);

export const TRADITIONAL_AUTHORS = Object.values(Author)
  .filter(author => !CMPC_SPECIFIC_AUTHORS.includes(author));
